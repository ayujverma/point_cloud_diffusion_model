#!/bin/bash
#===========================================================================
# Local Test Script — Mean Shape Computation (Template Selection)
#===========================================================================
#
# Runs compute_mean_shape.py locally to select the most central training
# shape as the template. Uses Chamfer distance to find the shape with
# lowest average distance to a random probe subset.
#
# Usage:
#   bash scripts/test_mean_shape.sh
#===========================================================================

# Change to repo directory
REPO_DIR="/work/10692/ayuj/point_cloud_diffusion_model"
cd "${REPO_DIR}"

DATA_ROOT="$REPO_DIR/data/human"
OUTPUT_DIR="$REPO_DIR/results/mean_shape"
OUTPUT_FILE="${OUTPUT_DIR}/mean_shape_test.npy"
N_POINTS=2048
N_PROBE=50              # Compare against 50 random shapes (fast for testing)

echo "==========================================="
echo " TESTING — Template Selection (Most Central Shape)"
echo "==========================================="
echo " Data root:  $DATA_ROOT"
echo " N points:   $N_POINTS"
echo " N probe:    $N_PROBE"
echo " Output:     $OUTPUT_FILE"
echo "==========================================="

# Check if training data exists
if [ ! -d "${DATA_ROOT}/train" ]; then
    echo "ERROR: Training data not found at ${DATA_ROOT}/train"
    exit 1
fi

mkdir -p "$OUTPUT_DIR"

export PYTHONPATH="."
python compute_mean_shape.py \
    --data_root "$DATA_ROOT" \
    --n_points $N_POINTS \
    --output "$OUTPUT_FILE" \
    --n_probe $N_PROBE

# Quick sanity check on the output
if [ -f "$OUTPUT_FILE" ]; then
    python -c "
import numpy as np
arr = np.load('${OUTPUT_FILE}')
norms = np.linalg.norm(arr, axis=1)
print(f'  Shape:    {arr.shape}')
print(f'  Max norm: {norms.max():.4f}')
print(f'  Min norm: {norms.min():.4f}')
bbox = arr.max(axis=0) - arr.min(axis=0)
print(f'  Bbox:     {bbox}')
assert arr.shape == (${N_POINTS}, 3), f'Expected (${N_POINTS}, 3), got {arr.shape}'
assert norms.max() <= 1.01, f'Max norm {norms.max():.4f} > 1.01'
print('  ✓ All checks passed.')
"
else
    echo "ERROR: Output file not created"
    exit 1
fi

echo "==========================================="
echo " Test completed."
echo "==========================================="
